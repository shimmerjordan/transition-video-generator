...
- 编码/混音用内置 ffmpeg(`imageio-ffmpeg`),**无需另装系统 ffmpeg**;有 NVIDIA 卡时自动用 **NVENC** GPU 编码。
...
```bash
# 1) 装 CUDA 版 torch(RTX 50 系/Blackwell 用 cu128;其他卡换对应 cuXXX)
.venv/Scripts/python -m pip install torch torchvision --index-url https://download.pytorch.org/whl/cu128
# 2) 装 sam2
.venv/Scripts/python -m pip install sam2 huggingface_hub
# 3) 下载权重(国内用 hf-mirror)
HF_ENDPOINT=https://hf-mirror.com .venv/Scripts/python -c "from huggingface_hub import hf_hub_download as d; d('facebook/sam2.1-hiera-small','sam2.1_hiera_small.pt',local_dir='models/sam2')"
```
...
```bash
# 旧 Pillow pin 在 py3.13 编译失败,用 --no-deps 装(torch/PIL/opencv 已具备)
.venv/Scripts/python -m pip install --no-deps simple-lama-inpainting
# 首次运行自动从 GitHub release 下 big-lama.pt(~200MB,需可直连 github;
# 若 git 配了挂掉的代理:git config --global --unset http.proxy)
```

「去水印」页「修复引擎」选 `lama`(默认):GPU、无幻觉、~0.1s/帧(裁剪区域)。
`cv2` 快但大区域会涂抹;`sd` 为实验性(文字附近可能幻觉)。
...
| 阶段 | 做什么 | 运行的底层步骤 |
|---|---|---|
| **去水印**(第1步) | 画面**拖拽框选**水印/字幕;**作用范围**可选「全段」或**新增分段区间**(不同时间段水印位置不同时分别框选);原始/输出**等大并排**;**▶运行**→ `clean/<bg>.mp4` | dewatermark |
...
| cleanup(去水印/字幕) | ✅ **LaMa(GPU,推荐,无幻觉)** / cv2(快) / sd(实验) | 多轨道按时段框选;动态路人需 `product:*` |
| matte 抠像 | ✅ **SAM2(GPU)** | 按人物 seed_point 输出 p0/p1 alpha + 时序追踪;无 GPU 回退中值法 |
...
- **固定机位却报很大运镜位移**:主体占画面大、移动剧烈会污染相机估计(S2 首遍无 mask)。三脚架素材把 `camera.mode` 设 `locked` 即可跳过运镜处理。
- **SAM2 报 `cannot import name '_C'`**:无害,仅跳过可选后处理内核,不影响结果。
